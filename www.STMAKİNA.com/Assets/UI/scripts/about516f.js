    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.data) {
                ...
                var lang = (window.HurmaksanI18n && window.HurmaksanI18n.getLanguage)
                    ? window.HurmaksanI18n.getLanguage() : 'tr';
                var rawText = data.data.Description || '';
                if (lang === 'en') {
                    var engDesc = data.data.EngDescription;
                    if (engDesc && String(engDesc).trim() && engDesc !== '-') {
                        rawText = engDesc;
                    } else if (window.HurmaksanI18n && window.HurmaksanI18n.t) {
                        // EN versiyonu DB'de yoksa about.intro varsayılan çevirisi
                        var fallback = window.HurmaksanI18n.t('about.intro');
